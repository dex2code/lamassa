...
/**
 * Transfers the NFT from owner to another address.
 * @param to - The address of the new owner
 * @param tokenId - The NFT to transfer
 *
 * @remarks This function can be called by the current owner of the NFT only.
 * If tonek does not exist or the caller is not the owner, it fails.
 *
 **/
export function _transfer(to: string, tokenId: u256): void {
  assert(Storage.has(ownerKey(tokenId)), 'Nonexistent token');
  const from = Context.caller().toString();
  assert(_ownerOf(tokenId) == from, 'Unauthorized from');
  assert(to != '', 'Unauthorized to');
  _update(to, tokenId, from);
}
